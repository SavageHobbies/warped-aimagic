# PowerShell script to update Archon task statuses
...
if ($initResult) {
    ...
    Write-Host "`n🔧 Getting available tools..." -ForegroundColor Cyan
    ...
    if ($toolsResult -and $toolsResult.result -and $toolsResult.result.tools) {
        ...
        if ($taskTool) {
            Write-Host "`n📋 Found task management tool: $($taskTool.name)" -ForegroundColor Green
            ...
            Write-Host "`n🔄 Updating task statuses..." -ForegroundColor Cyan
            ...
            foreach ($task in $completedTasks) {
                ...
                if ($result -and -not $result.error) {
                    ...
                    Write-Host "     ⚠️ Error: $($result.error.message)" -ForegroundColor Yellow
                }
            }
        }
    }
...
Write-Host "`n✨ Task update process completed" -ForegroundColor Cyan
