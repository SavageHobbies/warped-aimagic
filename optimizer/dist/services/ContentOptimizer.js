...
class ContentOptimizer {
    async optimizeContent(productDetails, researchData) {
        utils_1.logger.info(`Optimizing content for: ${productDetails.title}`);
        try {
            ...
            utils_1.logger.info(`Content optimization completed for: ${productDetails.title}`);
            return optimizedContent;
        }
        catch (error) {
            utils_1.logger.error(`Content optimization failed for: ${productDetails.title}`, error);
            throw new Error(`Content optimization failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    generateOptimizedTitle(originalTitle, keywords) {
        ...
        const missingKeywords = keywords.filter(keyword => !titleWords.some(word => word.includes(keyword.toLowerCase())));
    ...
    generateOptimizedDescription(productDetails, researchData) {
    ...
    generateSellingPointsSection(productDetails, researchData) {
        const sellingPoints = this.generateSellingPoints(productDetails, researchData);
    ...
    generateSpecificationsSection(specifications) {
        const importantSpecs = Object.entries(specifications)
            .filter(([key, value]) => key.length > 2 &&
            value.length > 2 &&
    ...
    generateConditionSection(condition) {
        let conditionText = '';
    ...
    generatePricingSection(originalPrice, priceAnalysis) {
    ...
    generateSellingPoints(productDetails, researchData) {
        const sellingPoints = [];
        // Extract key features from title
        const titleWords = productDetails.title.toLowerCase().split(' ');
        const importantWords = titleWords.filter(word => word.length > 3 &&
