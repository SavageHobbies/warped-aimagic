...
class TemplateRenderer {
    async renderTemplate(optimizedContent, productDetails, templatePath) {
        utils_1.logger.info(`Rendering template for: ${productDetails.title}`);
        try {
            // Use a simple HTML template
            const html = this.generateHtmlTemplate(optimizedContent, productDetails);
            utils_1.logger.info(`Template rendering completed for: ${productDetails.title}`);
            return html;
        }
        catch (error) {
            utils_1.logger.error(`Template rendering failed for: ${productDetails.title}`, error);
            throw new Error(`Template rendering failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    generateHtmlTemplate(optimizedContent, productDetails) {
