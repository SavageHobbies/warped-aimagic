...
interface ExternalProductData {
  ...
  // Material & Construction - All Categories
  material?: string
...
interface DataSource {
  name: string
  priority: number
  fetchData: (upc: string) => Promise<ExternalProductData | null>
}
...
class ProductDataService {
   ...
   */
  async fetchProductData(upc: string): Promise<ExternalProductData | null> {
    if (!upc || upc === 'NO_UPC') {
      return null
    }
   ...
   */
  private async fetchFromUPCItemDB(upc: string): Promise<ExternalProductData | null> {
   ...
   */
  private async fetchFromUPCDatabase(upc: string): Promise<ExternalProductData | null> {
   ...
   */
  private async fetchFromAmazonAPI(upc: string): Promise<ExternalProductData | null> {
    ...
    try {
      ...
      const catalogTimeoutId = setTimeout(() => catalogController.abort(), 20000)
      ...
      if (catalogData.items && catalogData.items.length > 0) {
        ...
        // Debug: Log item structure to understand image format
        console.log(`🔍 Amazon item structure:`, {
          hasImages: !!item.images,
          imagesLength: item.images?.length || 0,
          imagesStructure: item.images,
          asin: item.asin
        })
        ...
        return {
          ...
          images: item.images?.map((img: any) => img.link).filter(Boolean) || [],
          specifications: {
            ASIN: item.asin,
            UPC: upc,
            Brand: attributes.brand?.[0]?.value,
   ...
   */
  private determineProductCategory(upc: string): any {
   ...
   */
  private async fetchFromEbayAPI(upc: string): Promise<ExternalProductData | null> {
   ...
   */
  private async getEbayAccessToken(): Promise<string> {
    const appId = process.env.EBAY_APP_ID
   ...
   */
  mergeProductData(existingData: any, externalData: ExternalProductData): any {
   ...
   */
  getDataSourceStatus(): { name: string; configured: boolean; priority: number }[] {
   ...
   */
  private isDataSourceConfigured(sourceName: string): boolean {
    switch (sourceName) {
      case 'UPCItemDB':
